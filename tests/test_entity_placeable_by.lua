...
test("biterport has formation storage and a hidden robot service cell", function()
  ...
  local blocker = assert(find_entity_prototype("biterport-wall-blocker"))

  assert_eq(port.placeable_by[1].item, "biterport", "biterport should be placeable by its item")
  assert_eq(port.inventory_size, 9, "biterport should expose one money slot plus eight formation slots")
  assert_eq(port.inventory_type, "with_filters_and_bar", "biterport inventory should support formation slot filters")
  assert_true(port.collision_mask.layers.administratorio_station_footprint, "biterport should reserve its full placement footprint")
  assert_true(not port.collision_mask.layers.object, "biterport anchor should leave interior walkable")
  assert_eq(port.selection_box[1][1], -2.5, "biterport should expose the new 5x5 footprint")
  assert_eq(port.selection_box[2][1], 2.5, "biterport should expose the new 5x5 footprint")
  assert_true(port.stateless_visualisation ~= nil, "biterport should draw a roof overlay above workers")

  assert_true(blocker.selectable_in_game == false, "biterport wall blocker should stay hidden")
  assert_true(blocker.collision_mask.layers.object, "biterport wall blocker should block pathing")
...
test("biter station inventory supports worker slot filters", function()
  ...
  local blocker = assert(find_entity_prototype("biter-station-wall-blocker"))
  assert_eq(station.inventory_size, 20, "biter station base inventory size")
  assert_eq(station.inventory_type, "with_filters_and_bar", "biter station inventory should support funding and worker filters")
  assert_true(station.collision_mask.layers.administratorio_station_footprint, "biter station should reserve its 5x5 footprint")
  assert_true(not station.collision_mask.layers.object, "biter station anchor should leave its interior walkable")
  assert_eq(station.selection_box[1][1], -2.5, "biter station should use a 5x5 footprint")
  assert_eq(station.selection_box[2][1], 2.5, "biter station should use a 5x5 footprint")
  assert_true(station.stateless_visualisation ~= nil, "biter station should draw a roof overlay above workers")
  assert_true(blocker.collision_mask.layers.object, "biter station wall blocker should block pathing")
end)
...
test("biterport worker speed tiers use hidden custom biter units", function()
  ...
  assert_eq(base.collision_box[1][1], -0.18, "base worker should have a narrow collision box")
  assert_eq(express.collision_box[2][2], 0.18, "express worker should keep the narrow collision box")
  ...
  assert_true(base.run_animation.layers ~= nil, "base biterport worker should use layered animation")
  local helmet = base.run_animation.layers[#base.run_animation.layers]
  assert_eq(helmet.filenames[1], "__administratorio__/graphics/entities/worker-biter/biter-helmet-1.png", "base worker should render the helmet overlay")
  assert_eq(helmet.width, 177, "helmet overlay should use normalized frame width")
  assert_eq(helmet.height, 138, "helmet overlay should use normalized frame height")
  assert_eq(helmet.frame_count, 16, "helmet overlay should match biter walk frame count")
  assert_eq(helmet.direction_count, 16, "helmet overlay should match biter walk direction count")
  assert_true(fast.run_animation.layers[#fast.run_animation.layers].filenames[1] == helmet.filenames[1], "fast worker should render the helmet overlay")
  assert_true(express.run_animation.layers[#express.run_animation.layers].filenames[1] == helmet.filenames[1], "express worker should render the helmet overlay")
end)

test("paperwork logistic chests are one-slot robot-compatible biter chests", function()
  local provider = data.raw["logistic-container"]["paperwork-provider-chest"]
  local storage = data.raw["logistic-container"]["paperwork-storage-chest"]
  local requester = data.raw["logistic-container"]["paperwork-requester-chest"]

  assert_true(provider ~= nil, "paperwork provider chest missing")
  assert_true(storage ~= nil, "paperwork storage chest missing")
  assert_true(requester ~= nil, "paperwork requester chest missing")
  assert_eq(provider.logistic_mode, "passive-provider")
  assert_eq(storage.logistic_mode, "storage")
  assert_eq(requester.logistic_mode, "requester")
  assert_eq(provider.inventory_size, 1, "paperwork provider should be intentionally tiny")
  assert_eq(storage.inventory_size, 1, "paperwork storage should be intentionally tiny")
  assert_eq(requester.inventory_size, 1, "paperwork requester should be intentionally tiny")
