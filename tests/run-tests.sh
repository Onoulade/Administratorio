...
run_python_tests() {
  for test_file in "$TEST_DIR"/test_*.py; do
    ...
    if [ "$test_name" = "test_progression_report.py" ] || [ "$test_name" = "test_planet_escape.py" ] || [ "$test_name" = "test_factorio_config_matrix.py" ] || [ "$test_name" = "test_factorio_runtime_smoke.py" ] || [ "$test_name" = "test_factorio_biterport_smoke.py" ] || [ "$test_name" = "test_factorio_complaint_identity.py" ]; then
