...
If <version> is newer than info.json, updates info.json and CHANGELOG.md for a
release, creates a release commit, optionally adds an annotated tag, and builds
a Factorio zip archive next to the current mod folder.
...
[ -f "$CHANGELOG" ] || die "Missing CHANGELOG.md in $REPO_ROOT"
...
python3 - "$INFO_JSON" "$CHANGELOG" "$NEW_VERSION" "$RELEASE_DATE" "$NOTES_FILE" "$RELEASE_MODE" <<'PY'
...
if release_mode == "fresh":
    ...
    unreleased_end = unreleased_match.end() + next_section_match.start() if next_section_match else len(changelog_text)
    ...
    if next_section_match:
        updated_changelog += "\n\n" + changelog_text[unreleased_end:].lstrip("\n")
    else:
        updated_changelog += "\n"
