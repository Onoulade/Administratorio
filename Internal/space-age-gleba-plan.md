...
This file records the planned Gleba principles under the shared rules in [space-age-compatibility-plan.md](space-age-compatibility-plan.md).
...
Both `Capture Bureau` and `Conciliation Desk` require a `conciliation-officer` worker to craft but no operating paperwork. The yellow paperwork branch is unlocked by pre-agricultural `gleba-yellow-administration`; the profession and buildings are unlocked by the follow-up `gleba-conciliation` tech and remain Nauvis-only, like every other biter profession. Players bring finished officers while reusable MMMMs can be re-briefed cheaply on Gleba.
...
- `gleba-yellow-administration` establishes yellow forms, then `gleba-conciliation` unlocks the Nauvis officer formation before the Biochamber and agricultural science that consume the resulting specialist and eggs.
