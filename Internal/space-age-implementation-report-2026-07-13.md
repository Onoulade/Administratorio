...
- The Archive Recombination Bureau supports all 465 unordered pairs across 31 selected documents. Factorio furnaces have a hard one-source-slot limit, so the Bureau uses a Lab-derived multi-input inventory with the normal machine interface and a hidden 1 MW secondary-input power sink while processing. It automatically detects matching pairs and ejects results from its east side. Every pair has two or three technology-filtered candidates; attempts consume two different forms, succeed 50% of the time, preserve quality, and yield nothing on failure.
- A Relay Clerk is consumed in Bureau construction as its permanent employee. There is no custom Bureau GUI, manual pair registration, archival substrate, residue loop, or archive-specific data/excuse recovery recipe.
...
- Live Bureau smoke test: with no research active, a powered Bureau consumed two compatible forms, ran automatically, removed its temporary power sink on completion, and ejected the successful result onto an east-side belt.
- `git diff --check`: pass.

## Balance posture

The implemented numbers are conservative starting values. The Archive Bureau remains a 75% expected net form sink before candidate usefulness: three attempts per minute consume six forms and yield 1.5 forms on average. Old Archives now process in 0.5 seconds and average one starter form with no rubble byproduct. Every paperwork form can be shredded back into one paper at 25% probability. Scrap adds 0.70 expected rubble, 0.12 toner, 0.08 documentation, and a 6% archive chance per scrap operation.
