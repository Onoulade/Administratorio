...
- the Orbital Miner Deployment Catapult (`orbital-employment-catapult`) has 56-tile reach through a narrow 18° firing corridor; it launches `voluntary-exploration-space-miner` projectiles that attach visibly and perform research-scalable mining damage on quarter-second work cycles, reserves exact asteroid capacity at launch, excludes staffed/reserved asteroids from deviation, and creates the full 2/6/18/54 collectible yield after demolition
...
- each asteroid begins with a hard allocation of one attached miner; four staffing-capacity researches raise it to five, capped catapults pause or retarget, and already-airborne overflow returns as a collectible employee chunk
