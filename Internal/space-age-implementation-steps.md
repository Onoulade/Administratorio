...
- the Orbital Miner Deployment Cannon (`orbital-employment-cannon`) retains 48-tile reach but uses a narrow 36° firing corridor; it launches `voluntary-exploration-space-miner` projectiles that attach visibly and perform research-scalable mining damage on one-second work cycles, reserves exact asteroid capacity at launch, excludes staffed/reserved asteroids from deviation, and creates the full 2/6/18/54 collectible yield after demolition
